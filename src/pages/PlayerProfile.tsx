...
function StatRow({ label, value, highlight }: StatRowProps) {
  return (
    <div className="flex items-center justify-between py-3 border-b border-gray-800 last:border-0">
      ...
      <span className={`font-semibold text-sm ${highlight ? 'text-blue-400' : 'text-white'}`}>
        {value}
      </span>
    </div>
  )
      ...
      <div className="max-w-3xl mx-auto px-6 py-10 space-y-10">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-500 text-sm mt-0.5">
                {playerStats.games_played} {playerStats.games_played === 1 ? 'game' : 'games'} played
              </p>
            </div>
            {myAwards.length > 0 && (
              ...
              {myAwards.map(award => (
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-gray-500 text-xs mt-0.5">{award.value}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <section className="space-y-3">
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-2xl px-5">
            <StatRow label="Goals" value={playerStats.goals} highlight={playerStats.goals > 0} />
            <StatRow label="Assists" value={playerStats.assists} highlight={playerStats.assists > 0} />
            ...
            {playerStats.hat_tricks > 0 && (
              <StatRow label="Hat Tricks 🎩" value={playerStats.hat_tricks} highlight />
            )}
            <StatRow
              label="Current Scoring Streak"
              value={playerStats.current_scoring_streak > 0
            ...
            <StatRow
              ...
              value={playerStats.best_scoring_streak > 0
            ...
            {bestGame && bestGame.goal_involvements > 0 && (
              <StatRow
                label="Best Game"
                value={`${bestGame.goal_involvements} G+A (${new Date(bestGame.game.date).toLocaleDateString('en-GB', {
                  day: 'numeric',
                  month: 'short',
                })})`}
                highlight
        ...
        <section className="space-y-3">
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-2xl px-5">
            ...
            <StatRow
              label="Shot Accuracy"
              value={playerStats.shots_on_target + playerStats.shots_off_target > 0
        ...
        <section className="space-y-3">
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-2xl px-5">
            <StatRow label="Wins" value={playerStats.wins} highlight={playerStats.wins > 0} />
            <StatRow label="Losses" value={playerStats.losses} />
            <StatRow label="Draws" value={playerStats.draws} />
            <StatRow label="Win Rate" value={`${winRate}%`} highlight={winRate >= 50} />
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-800">
                      ...
                      <th className="text-center px-4 py-3 text-gray-500 font-semibold text-xs uppercase tracking-wider">G</th>
                      <th className="text-center px-4 py-3 text-gray-500 font-semibold text-xs uppercase tracking-wider">A</th>
                      <th className="text-center px-4 py-3 text-gray-500 font-semibold text-xs uppercase tracking-wider">G+A</th>
                      <th className="text-center px-4 py-3 text-gray-500 font-semibold text-xs uppercase tracking-wider">SOT</th>
                      <th className="text-center px-4 py-3 text-gray-500 font-semibold text-xs uppercase tracking-wider">KP</th>
                      <th className="text-center px-4 py-3 text-gray-500 font-semibold text-xs uppercase tracking-wider">Result</th>
                    </tr>
                  </thead>
                  <tbody>
                    {gameBreakdown.map(({ game, goals, assists, key_passes, shots_on_target, goal_involvements }) => {
                      const gp = gamePlayers.find(
                        g => g.game_id === game.id && g.player_id === id
                      )
                      const playerTeam = gp?.team
                      const result = game.winning_team === null
                        ? '—'
                        : game.winning_team === 0
                          ? 'D'
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className={`text-center px-4 py-3.5 font-semibold ${resultColor}`}>
                            {result}
                          </td>
                        </tr>
                      )
