...
type SortKey = 'goals' | 'assists' | 'shots_on_target' | 'key_passes' | 'shot_accuracy' | 'shot_conversion' | 'goals_per_game' | 'wins' | 'losses' | 'draws'
...
const HEADERS: { key: SortKey; label: string; tooltip: string }[] = [
    ...
    { key: 'shot_accuracy', label: 'ShAcc%', tooltip: 'Shot Accuracy (shots on target / total shots)' },
    { key: 'shot_conversion', label: 'Conv%', tooltip: 'Shot Conversion (goals / total shots)' },
                        ...
                        <tbody>
                            {sorted.map((s, i) => (
                                ...
                                >
                                    ...
                                    <td className="text-center px-4 py-3.5 text-white font-semibold">{s.goals}</td>
                                    <td className="text-center px-4 py-3.5 text-gray-300">{s.assists}</td>
                                    <td className="text-center px-4 py-3.5 text-gray-300">{s.shots_on_target}</td>
                                    <td className="text-center px-4 py-3.5 text-gray-300">{s.key_passes}</td>
                                    ...
                                    <td className="text-center px-4 py-3.5 text-green-400">{s.wins}</td>
                                    <td className="text-center px-4 py-3.5 text-red-400">{s.losses}</td>
                                    <td className="text-center px-4 py-3.5 text-gray-400">{s.draws}</td>
                                        ...
                                        <div className="flex items-center justify-center gap-0.5">
                                            {s.form.length === 0 ? (
                                                ...
                                                s.form.map((result, i) => (
                                                    <span
                                                        key={i}
                                                        className={`text-xs font-bold w-5 h-5 rounded flex items-center justify-center ${result === 'W' ? 'bg-green-900/60 text-green-400' :
